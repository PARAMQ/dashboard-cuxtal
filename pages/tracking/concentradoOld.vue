...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (zone) {
          temporalBinnacles = resTemporal.filter((x) => {
            if (x.list_subzones) {
              const temporal = x.list_subzones.filter(
                (x) => x.idzoning === zone
              )
              if (temporal.length > 0) {
                return x
        ...
        if (temporalBinnacles.length > 0) {
          for (let i = 0; i < 12; i++) {
            ...
            this.seriesBinnaclesPerMonth[0].data.push(
              parseInt(temporalFilter.length)
            )
          }
    ...
    async getData () {
      ...
      const temporalYear = this.selectYear
        ? this.selectYear.fecha_captura
        : null
