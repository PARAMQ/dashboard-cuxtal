                ...
                <b-field label="Participantes">
                  ...
                  >
                    <template slot-scope="props">
                      <strong>{{ props.option.name }} {{ props.option.lastname }}</strong>
                    </template>
                    <template #empty>
                      Sin resultados
                    </template>
...
export default {
  ...
  watch: {
    plannification (newVal, oldVal) {
      if (newVal) {
        this.form.idplanification = newVal
      }
    }
  },
  mounted () {
  ...
  methods: {
        ...
        if (this.files.length > 0) {
          ...
          this.files.forEach((files, index) => {
            this.temporalFiles.push({
              description: 'evidencia_' + (index + 1),
    ...
    addPoint () {
      if (this.formCoord.name && this.formCoord.name !== '') {
        if (this.points.length === 0) {
          if (this.isSwitched) {
            this.convertCoordinatesToUtm([this.temporalPoint[0], this.temporalPoint[1]])
            const temporalPoints = [
              [this.temporalPoint[0], this.temporalPoint[1]]
            ]
